...
public class BinarySearchTreeNode<T extends Comparable<T>> {
  ...
  private BinarySearchTreeNode<T> removeThis() {
    ...
    // root node to remove has 2 children
      ...
      this.data = succ.data;
      succ.data = tempData;
      right = right.remove(tempData);
    }
    return this;
  }

  private BinarySearchTreeNode<T> removeLeft() {
    ...
    // node to remove has 2 children
      ...
      var tempData = toRemove.data;
      left.data = succ.data;
      succ.data = tempData;
      left.right = left.right.remove(tempData);
    }
    return this;
  }

  private BinarySearchTreeNode<T> removeRight() {
    ...
    // node to remove has 2 children
      ...
      succ.data = tempData;
      right.right = right.right.remove(tempData);
    }
    return this;
  }
  ...
  public BinarySearchTreeNode<T> remove(T data) {
    ...
    int res = data.compareTo(this.data);
    boolean thisEquals = equals(data, this.data);
    boolean leftEquals = left != null && equals(data, left.data);
    boolean rightEquals = right != null && equals(data, right.data);
    if (thisEquals && size() == 1) {
      return null;
    ...
    else if (right != null && res > 0)
      right = right.remove(data);
    else if (left != null && res < 0)
